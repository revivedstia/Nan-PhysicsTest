...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label5->Text = L"Ïîâòîðèòå ïàðîëü";
			// 
			// txtPassword2
			// 
			...
			this->cmd_to_login->Text = L"Âîéòè";
			...
			this->label4->Text = L"Ó ìåíÿ óæå åñòü àêêàóíò";
			// 
			// cmd_signin
			// 
			...
			this->cmd_signin->Text = L"Çàðåãèñòðèðîâàòüñÿ";
			...
			this->label1->Text = L"Ðåãèñòðàöèÿ ïîëüçîâàòåëÿ";
			// 
			// txtLogin
			// 
			...
			this->Text = L"Òåñò ïî íàíîôèçèêå - Ðåãèñòðàöèÿ ïîëüçîâàòåëÿ";
...
private: System::Void cmd_signin_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	if (name == "") {
		MessageBox::Show("Ïîëå \"Èìÿ\" íå çïîëíåíî", "Ñîîáùåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
		return;
	}

	if (login == "") {
		MessageBox::Show("Ïîëå \"Ëîãèí\" íå çïîëíåíî", "Ñîîáùåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
		return;
	}

	if (pass == "") {
		MessageBox::Show("Ïîëå \"Ïàðîëü\" íå çïîëíåíî", "Ñîîáùåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
		return;
	}

	if (pass2 == "") {
		MessageBox::Show("Ïîëå \"Ïîâòîðèòå ïàðîëü\" íå çïîëíåíî", "Ñîîáùåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
		return;
	}
	if (pass != pass2) {
		MessageBox::Show("Ïàðîëè íå ñîâïàäàþò", "Ñîîáùåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
		return;
	}
	if (Class1::UserIndex("users.txt", login) != -1) {
		MessageBox::Show("Ïîëüçîâàòåëü ñ òàêèì ëîãèíîì óæå ñóùåñòâóåò", "Ñîîáùåíèå", MessageBoxButtons::OK, MessageBoxIcon::Information);
		return;
	}
	String^ ind = Convert::ToString(Class1::LastIndex("users.txt") + 1);
	Class1::PrinInFile("users.txt", ind + ";" + login + ";" + name + ";" + pass + ";0");
	MessageBox::Show("Ïîëüçîâàòåëü óñïåøíî çàðåãèñòðèðîâàí", "",
